...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        //------------------------------------------------------------------------
        // Presence Haptics API

        /// <summary> Plays a Force effect to all the fingers of a hand that is touching / grabbing this object </summary>
        public void PlayStiffnessEffect(Presence.PHap_HapticEffect effect)
        {
            if (effect.EffectType != Presence.PHap_HapticModality.Stiffness)
                return;

            GrabArguments[] sendTo;
            if (this.grabbedBy.Count == 0)
                sendTo = new GrabArguments[1] { this.LastGrabbedBy };
            else 
                sendTo = this.grabbedBy.ToArray();

            for (int i = 0; i < sendTo.Length; i++)
            {
                if (sendTo[i] != null && sendTo[i].TrackedHand != null)
                {
                    bool rightHand = sendTo[i].TrackedHand.TracksRightHand();
                    Presence.PHap_Core.PlayHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightThumb         : Presence.PHap_BodyPart.LeftThumb ));
                    Presence.PHap_Core.PlayHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightIndexFinger   : Presence.PHap_BodyPart.LeftIndexFinger ));
                    Presence.PHap_Core.PlayHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightMiddleFinger  : Presence.PHap_BodyPart.LeftMiddleFinger ));
                    Presence.PHap_Core.PlayHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightRingFinger    : Presence.PHap_BodyPart.LeftRingFinger ));
                    Presence.PHap_Core.PlayHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightPinky         : Presence.PHap_BodyPart.LeftPinky ));
                }
            }
        }
        ...
        /// <summary> Plays a Force effect to all the fingers of a hand that is touching / grabbing this object </summary>
        public void StopStiffnessEffect(Presence.PHap_HapticEffect effect)
        {
            if (effect.EffectType != Presence.PHap_HapticModality.Stiffness)
                return;
            ...
            for (int i = 0; i < sendTo.Length; i++)
            {
                ...
                {
                    bool rightHand = sendTo[i].TrackedHand.TracksRightHand();
                    Presence.PHap_Core.StopHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightThumb         : Presence.PHap_BodyPart.LeftThumb ));
                    Presence.PHap_Core.StopHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightIndexFinger   : Presence.PHap_BodyPart.LeftIndexFinger ));
                    Presence.PHap_Core.StopHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightMiddleFinger  : Presence.PHap_BodyPart.LeftMiddleFinger ));
                    Presence.PHap_Core.StopHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightRingFinger    : Presence.PHap_BodyPart.LeftRingFinger ));
                    Presence.PHap_Core.StopHapticEffect(effect, new Presence.PHap_EffectLocation( rightHand ? Presence.PHap_BodyPart.RightPinky         : Presence.PHap_BodyPart.LeftPinky ));
                }
            }
        }
